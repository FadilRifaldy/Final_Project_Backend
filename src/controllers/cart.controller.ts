  ...
  try {
    ...
    const formattedItems = await Promise.all(
      cart.items.map(async (item) => {
        ...
        // Get primary image
        const primaryImage = item.productVariant.assignedImages.find((img) => img.isPrimary);
...
export async function addItemToCart(req: Request, res: Response) {
  try {
    ...
    const totalQuantity = updatedCart?.items.reduce((sum, item) => sum + item.quantity, 0) || 0;
